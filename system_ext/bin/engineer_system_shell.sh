...
function doSwitchEng {
    ...
    if [[ ${is_mtk} ]] ; then
        ...
        sys_diag_adb=`getprop sys.usb.config`
        engineer_diag_adb=`getprop vendor.oplus.engineer.usb.config`
        if [[ ${sys_diag_adb} != "diag,adb" ]]; then
                setprop sys.usb.config diag,adb
        fi
        if [[ ${engineer_diag_adb} != "diag,adb" ]]; then
